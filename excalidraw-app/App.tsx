...
const ExcalidrawWrapper = () => {
  ...
  // ── Ribeiro Draw!: Auto-open AWS Icons sidebar on load ─────────────────────
  useEffect(() => {
    if (!excalidrawAPI) {
      return;
    }
    const timer = setTimeout(() => {
      excalidrawAPI.updateScene({
        appState: {
          openSidebar: { name: "default", tab: "aws-icons" },
        },
      });
    }, 600);
  ...
  // ── Ribeiro Draw!: Handle AWS icon drag-and-drop onto canvas ──────────────
  useEffect(() => {
    if (!excalidrawAPI) {
      return;
    }
    ...
    const handleDragOver = (e: DragEvent) => {
      if (e.dataTransfer?.types.includes("application/aws-icon-data")) {
        e.preventDefault();
